...
// additions required for sunspot-wasm-bridge

// GetWitnessFromReader does the exact same thing as GetWitness, but from memory!
func (acir *ACIR[T, E]) GetWitnessFromReader(r io.Reader, field *big.Int) (witness.Witness, error) {
	// 1. Call the Stack Loader!
	witnesses, err := LoadWitnessStacksFromReader[T](r, field)
	if err != nil {
		return nil, err
	}
...
// LoadWitnessStacksFromReader loads the witness stacks from an io.Reader (memory-safe)
func LoadWitnessStacksFromReader[T shr.ACIRField](r io.Reader, modulus *big.Int) (WitnessStacks[T], error) {
